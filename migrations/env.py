"""Alembic async environment (ported from Boxly's ``migrations/env.py``).

Imports ``vitals.models`` so autogenerate sees every table on ``Base.metadata``.
The DB URL is taken from ``VITALS_DATABASE_URL`` at runtime, overriding the
...
database_url = os.environ.get("VITALS_DATABASE_URL")
if database_url:
