...
У Vitals есть **голос**: проактивный слой сам собирает утренний бриф и подсказки по условию, вместо того чтобы ждать, пока в дашборд зайдут. Единственная активная поверхность доставки проактивного текста пока — `/reports`. PWA отправляет сообщения команды заботы через зашифрованную регистрацию устройств, subject-isolated outbox и consent-rechecking PHI-free Web Push; service worker показывает только нейтральный факт нового сообщения и ведёт во входящие без имени, текста, файла или медицинских данных.
...
Vitals has a **voice**: a proactive layer builds a morning brief and condition-driven nudges by itself rather than waiting for the dashboard to be opened. `/reports` remains the only active delivery surface for proactive text. The PWA sends care-team messages through encrypted device enrollment, a subject-isolated outbox, and consent-rechecking PHI-free Web Push; the service worker shows only the neutral fact that a message arrived and opens the inbox without a name, body, file, or health detail.
