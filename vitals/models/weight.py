...
class NoiseMarker(Base, SubjectOwnershipMixin, OriginActorMixin, TimestampMixin):
    ...
    # Which way did the scale move vs the real fat trend?
    # UP   = weight inflated (creatine, sodium) → real loss better than numbers
    # DOWN = weight deflated (dehydration)       → real situation worse
    # NULL (legacy rows) is treated as NEUTRAL by the digest context builder.
    direction: Mapped[Optional[str]] = mapped_column(
        String(16), nullable=True, server_default=None
    )
