...
async def _lock_prepared_scope(
    ...
    *,
    ...
    # Upload consumers use RawPayload as the provenance root and only then lock
    # its FileAsset.  Finalization must use the same order: taking FileAsset
    # first lets a concurrent confirm hold RawPayload and form a PostgreSQL
    # deadlock cycle.
...
def prepare_lab_document_content(
    ...
    *,
    ...
    try:
        image_urls = ingestion.prepare_file_for_extraction(
            file_bytes,
    ...
    *,
    ...
    async def provider_call(
        ...
        if prepared_content is not None and prepared_content._is_pdf:
            return await ingestion.extract_prepared_file_with_usage(
        ...
        return await ingestion.extract_from_file_with_usage(
            file_bytes,
            llm=client,
            content_type=snapshot._media_type,
            filename=snapshot._storage_ref,
