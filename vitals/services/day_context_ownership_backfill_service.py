"""Bounded Stage-3I ownership backfill for optional-channel day context.
...
DAY_CONTEXT_OWNERSHIP_BACKFILL_PHASE = "stage3.channel_optional.day_context.v1"
DAY_CONTEXT_OWNERSHIP_BACKFILL_TABLES = ("day_context",)
DAY_CONTEXT_OWNERSHIP_BACKFILL_CHECKPOINT_PHASES: Mapping[str, str] = (
    MappingProxyType(
        {
            "day_context": (
                f"{DAY_CONTEXT_OWNERSHIP_BACKFILL_PHASE}.day_context"
            )
        }
    )
)
DEFAULT_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE = 250
MAX_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE = 1000

_TABLE: Table = DayContext.__table__
_PHASE_KEY = DAY_CONTEXT_OWNERSHIP_BACKFILL_CHECKPOINT_PHASES["day_context"]
_PAGE_SIZE = 1000
...
class DayContextOwnershipBackfillStatus(StrEnum):
    NOT_STARTED = "not_started"
    RUNNING = "running"
    COMPLETED = "completed"


class DayContextOwnershipBackfillError(RuntimeError):
    """Base class for fail-closed Stage-3I errors."""


class DayContextOwnershipBackfillValidationError(
    DayContextOwnershipBackfillError, ValueError
):
    """A caller argument or persisted scalar is invalid."""


class DayContextOwnershipBackfillIdentityError(DayContextOwnershipBackfillError):
    """The exact-one reviewed owner graph is unavailable."""


class DayContextOwnershipBackfillDependencyError(
    DayContextOwnershipBackfillError
):
    """A prerequisite checkpoint is absent, malformed, or in the wrong mode."""


class DayContextOwnershipBackfillStateError(DayContextOwnershipBackfillError):
    """Checkpoint progress or an ownership root is inconsistent."""


class DayContextOwnershipBackfillProvenanceError(
    DayContextOwnershipBackfillError
):
    """A day-context row has unsupported persisted provenance."""


@dataclass(frozen=True, slots=True)
class DayContextOwnershipBackfillPreflightResult:
    phase_key: str
    subject_id: uuid.UUID
    status: DayContextOwnershipBackfillStatus
    tables_total: int
    ...
    def completed(self) -> bool:
        return self.status is DayContextOwnershipBackfillStatus.COMPLETED
...
@dataclass(frozen=True, slots=True)
class DayContextOwnershipBackfillBatchResult(
    DayContextOwnershipBackfillPreflightResult
):
    batch_table: str
    ...
    def to_safe_dict(self) -> dict[str, str | int]:
        result = DayContextOwnershipBackfillPreflightResult.to_safe_dict(self)
        result.update(
            {
...
def _validate_batch_size(value: object) -> int:
    if (
        ...
        or not 1 <= value <= MAX_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE
    ):
        raise DayContextOwnershipBackfillValidationError(
...
def _validate_checkpoint(checkpoint: Any, *, phase: str, subject_id: uuid.UUID) -> str:
    error = (
        DayContextOwnershipBackfillDependencyError
        if phase in _PRIOR_PHASES
        else DayContextOwnershipBackfillStateError
    )
...
async def _load_scope(session: AsyncSession, *, for_update: bool) -> _Scope:
    ...
    if len(rows) != 1:
        raise DayContextOwnershipBackfillIdentityError(
            "day-context backfill requires exactly one health subject"
        )
    ...
    def require(phases: tuple[str, ...], nonempty_status: str, label: str) -> None:
        for phase in phases:
            ...
            if not _exact_empty_completed(checkpoint):
                raise DayContextOwnershipBackfillDependencyError(
...
def _validate_own(checkpoint: Any | None, *, scope: _Scope) -> str | None:
    ...
    if status == "restore_blocked":
        raise DayContextOwnershipBackfillStateError(
            "Stage-3I checkpoints cannot be restore-blocked"
        )
    return status
...
def _require_dependencies(
    ...
    if set(checkpoints) != set(_PRIOR_PHASES):
        raise DayContextOwnershipBackfillDependencyError(
            "Stage-3A through Stage-3H checkpoints are incomplete"
        )
    statuses = {
    ...
    if not own_exists:
        raise DayContextOwnershipBackfillDependencyError(
            "restore-mode Stage-3I requires its exact portability checkpoint"
        )
...
def _canonical(value: Any) -> Any:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise DayContextOwnershipBackfillProvenanceError(
                "day context contains a non-finite JSON number"
            )
    ...
    if isinstance(value, Mapping):
        if any(type(key) is not str for key in value):
            raise DayContextOwnershipBackfillProvenanceError(
                "day-context JSON object keys must be strings"
            )
        return {key: _canonical(value[key]) for key in sorted(value)}
    if isinstance(value, (list, tuple)):
        return [_canonical(item) for item in value]
    raise DayContextOwnershipBackfillProvenanceError(
        "day context contains an unsupported JSON value"
    )
...
def _data_envelope(row: Any) -> list[Any]:
    return ["day_context", *[getattr(row, field) for field in _DATA_FIELDS]]
...
def _validate_json_shape(row: Any) -> None:
    if type(row.answers) is not dict:
        raise DayContextOwnershipBackfillProvenanceError(
            "day-context answers must be a JSON object"
        )
    if row.planned is not None and type(row.planned) is not dict:
        raise DayContextOwnershipBackfillProvenanceError(
            "day-context planned value must be a JSON object or null"
        )
    _canonical(row.answers)
    _canonical(row.planned)
...
def _validate_connection(connection: Any, *, scope: _Scope) -> None:
    if (
        ...
        raise DayContextOwnershipBackfillProvenanceError(
            "day context has invalid recipient connection provenance"
        )
...
def _validate_row(
    ...
    *,
    ...
    if row.domain != Domain.SIGNALS.value or row.source not in _ALLOWED_SOURCES:
        raise DayContextOwnershipBackfillProvenanceError(
            "day context has invalid domain or source"
        )
    if not isinstance(row.id, int) or isinstance(row.id, bool) or row.id <= 0:
        raise DayContextOwnershipBackfillValidationError(
            "day context has an invalid primary key"
        )
    if not isinstance(row.date, date):
        raise DayContextOwnershipBackfillProvenanceError(
            "day context has an invalid date"
        )
    _validate_json_shape(row)
    ...
    if roots == (None, None, None):
        if not allow_unowned:
            raise DayContextOwnershipBackfillStateError(
                "an unowned day context is outside the historical bridge"
            )
        return True
    if row.subject_id != scope.subject_id:
        raise DayContextOwnershipBackfillStateError(
            "day context has partial or foreign ownership roots"
        )
    if row.actor_user_id not in {None, scope.owner_user_id}:
        raise DayContextOwnershipBackfillStateError(
            "day context actor is outside the reviewed ownership boundary"
        )
    connection = None
    if row.integration_connection_id is not None:
        ...
        if connection is None:
            raise DayContextOwnershipBackfillStateError(
                "day context references a missing recipient connection"
            )
    ...
    if historical:
        return False
    if row.source == Source.TEMPLATE.value:
        valid = row.actor_user_id is None and connection is None
    elif row.source == Source.MCP.value:
        # Updating a Telegram-origin row through MCP intentionally preserves its
        # already-reviewed recipient root; set_day_context never erases C.
        valid = row.actor_user_id == scope.owner_user_id
    elif row.source == Source.TELEGRAM.value:
        valid = row.actor_user_id == scope.owner_user_id and connection is not None
    else:
        valid = row.actor_user_id == scope.owner_user_id
    if not valid:
        raise DayContextOwnershipBackfillProvenanceError(
            "live day context has an invalid source/actor/recipient shape"
        )
    return False


async def _after_day_context_projection_for_test() -> None:
...
async def _lock_projected_rows(
    ...
    if set(locked_rows) != set(projected_rows) or any(
        ...
        raise DayContextOwnershipBackfillStateError(
            "a projected day context changed before it was locked"
        )
    return locked_rows
...
async def _project_and_lock_ids(
    ...
    *,
    ...
    if invoke_race_hook:
        await _after_day_context_projection_for_test()
...
def _row_policy(row_id: int, checkpoint: Any | None) -> tuple[bool, bool]:
    """Return ``(historical, allow_unowned)`` for the current checkpoint."""
    ...
    if checkpoint.status == "completed":
        ...
        return False, False
    raise DayContextOwnershipBackfillStateError(
        "Stage-3I checkpoint has an unsupported state"
    )
    ...
    *,
    ...
    while True:
        ...
        if set(projected) != set(connection_ids):
            raise DayContextOwnershipBackfillStateError(
                "a day context references a missing recipient connection"
            )
        if lock_connections:
            await _lock_projected_connections(session, projected)
        for connection_id in connection_ids:
            digest = _extend(digest, ["day_context_connection", connection_id])
            count += 1
...
async def _scan_current(
    ...
    *,
    ...
    while True:
        ...
        if set(rows) != set(ids):
            raise DayContextOwnershipBackfillStateError(
                "a projected day-context page changed during validation"
            )
        for row_id in ids:
            row = rows[row_id]
            ...
            if needs_adoption and checkpoint is not None and (
                ...
                raise DayContextOwnershipBackfillStateError(
                    "a processed day-context row remained unowned"
                )
            if digest:
    ...
    if for_update:
        ...
        if (
            ...
            raise DayContextOwnershipBackfillStateError(
                "day-context recipient references changed during validation"
            )
...
async def _bounds(session: AsyncSession) -> tuple[int, int]:
    ...
    if not _valid_counter(high) or not _valid_counter(count) or count > high:
        raise DayContextOwnershipBackfillValidationError(
            "day-context snapshot bounds are invalid"
        )
    return high, count
...
async def _status_result(
    ...
    *,
    ...
    if checkpoint is None:
        ...
        status = DayContextOwnershipBackfillStatus.NOT_STARTED
        ...
        status = DayContextOwnershipBackfillStatus(checkpoint.status)
        ...
        completed = status is DayContextOwnershipBackfillStatus.COMPLETED
    if validate:
        await _scan_current(
            session,
            scope=scope,
        ...
        if checkpoint is not None:
            ...
            if frozen_count != snapshot:
                raise DayContextOwnershipBackfillStateError(
                    "the day-context snapshot cardinality changed"
                )
    return DayContextOwnershipBackfillPreflightResult(
        phase_key=DAY_CONTEXT_OWNERSHIP_BACKFILL_PHASE,
...
def _batch_result(
    result: DayContextOwnershipBackfillPreflightResult,
    *,
    scanned: int,
    updated: int,
    unchanged: int,
) -> DayContextOwnershipBackfillBatchResult:
    return DayContextOwnershipBackfillBatchResult(
        **{
            field: getattr(result, field)
            for field in DayContextOwnershipBackfillPreflightResult.__dataclass_fields__
        },
        batch_table="day_context",
...
async def preflight_day_context_ownership_backfill(
    session: AsyncSession,
) -> DayContextOwnershipBackfillPreflightResult:
    """Validate the fixed Stage-3I graph without mutation."""
...
def _validate_restore_bounds(snapshot_bounds: Any) -> tuple[int, int]:
    if (
        ...
        or set(snapshot_bounds) != {"day_context"}
    ):
        raise DayContextOwnershipBackfillValidationError(
            "snapshot_bounds must contain the exact day-context table catalog"
        )
    pair = snapshot_bounds["day_context"]
    if not isinstance(pair, tuple) or len(pair) != 2:
        raise DayContextOwnershipBackfillValidationError(
            "the day-context snapshot bound must be an exact pair"
        )
    high, count = pair
    if (
        ...
        raise DayContextOwnershipBackfillValidationError(
            "the day-context snapshot bound is an invalid ID/count pair"
        )
    return high, count


async def reset_day_context_ownership_backfill_for_portability_v1_restore(
    session: AsyncSession,
    *,
    snapshot_bounds: Mapping[str, tuple[int, int]],
) -> None:
    """Reset Stage-3I before the caller atomically replaces portable data."""
...
def _set_cached_subject(
    ...
    cached = session.identity_map.get((DayContext, (row_id,), None))
    if cached is not None:
        attributes.set_committed_value(cached, "subject_id", subject_id)


async def run_day_context_ownership_backfill_batch(
    session: AsyncSession,
    *,
    batch_size: int = DEFAULT_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE,
) -> DayContextOwnershipBackfillBatchResult:
    """Advance the fixed day-context table by at most one primary-key batch."""
    ...
    with session.no_autoflush:
        ...
        rows, connections = await _project_and_lock_ids(
        ...
        for row_id in ids:
            ...
            if needs_adoption:
                ...
                if result.rowcount != 1:
                    raise DayContextOwnershipBackfillStateError(
                        "day-context ownership changed during adoption"
                    )
            ...
            if current_result is None:
                raise DayContextOwnershipBackfillStateError(
                    "a day context disappeared during adoption"
                )
            ...
            if (
                ...
                raise DayContextOwnershipBackfillStateError(
                    "day-context recipient changed during adoption"
                )
            if _validate_row(
                current,
                scope=scope,
        ...
        if remaining == 0:
            ...
            if (
                ...
                raise DayContextOwnershipBackfillStateError(
                    "the day-context snapshot changed during finalization"
                )
...
__all__ = [
    ...
    "DEFAULT_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE",
    "MAX_DAY_CONTEXT_OWNERSHIP_BACKFILL_BATCH_SIZE",
    "DayContextOwnershipBackfillStatus",
    "DayContextOwnershipBackfillError",
    "DayContextOwnershipBackfillValidationError",
    "DayContextOwnershipBackfillIdentityError",
    "DayContextOwnershipBackfillDependencyError",
    "DayContextOwnershipBackfillStateError",
    "DayContextOwnershipBackfillProvenanceError",
    "DayContextOwnershipBackfillPreflightResult",
    "DayContextOwnershipBackfillBatchResult",
    "preflight_day_context_ownership_backfill",
    "run_day_context_ownership_backfill_batch",
