...
| `professional_profiles` / `ProfessionalProfile` | Account control plane | One professional claim per user. Its relationship to a patient names whether that person acts as doctor or trainer. Verification records the reviewing operator and time; rejection or suspension retains a bounded reason. A role or verified profile alone never grants PHI access. |
| `professional_review_decisions` / `ProfessionalReviewDecision` | Account control plane | Append-only operator decision history records the exact professional-profile transition, reviewer, time, and bounded refusal reason. It is internal governance evidence, not PHI authorization or an ordinary-user portability row. |
| `registration_invitations` / `RegistrationInvitation` | Account control plane | An expiring invitation stores only a lowercase SHA-256 token digest and normalized address for one non-privileged account kind. One live invite per address is enforced, so two credentials cannot disagree about the account to create. Terminal purge removes applicant and user links while retaining only the opaque outcome and timestamps. The schema does not itself open registration. |
| `registration_intents` / `RegistrationIntent` | Account control plane | A short-lived public-registration choice stores only an opaque UUID, one non-privileged account kind, and lifecycle timestamps. It carries no email, provider identity, credential, or health data; it is consumed atomically with account provisioning and is excluded from user portability archives. |
| `registration_requests` / `RegistrationRequest` | Platform control plane | One live request keys the exact provider issuer/subject pair; terminal attempts remain separate history. Every pending, approved, rejected, or expired outcome is constrained. Approval requires an accountable reviewer and provisioned account; rejection requires a bounded reason. Terminal purge scrubs applicant PII, provider identity, reason, and user links while retaining the opaque outcome. The schema does not itself open registration. |
