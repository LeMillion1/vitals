"""Non-PHI operator contract for the fixed Stage-3O body-scan CLI."""
...
from scripts import backfill_body_scan_subject_ownership as cli
...
PHI_SENTINEL = "private-body-scan-health-sentinel"
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.BODY_SCAN_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.BODY_SCAN_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_allowlisted_counts(
    ...
    assert payload["operation"] == "body_scan_subject_ownership_backfill"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.file_backed.body_scans.v1"],
        ["--table", "body_scans"],
...
@pytest.mark.parametrize(
    ...
    [
        ("BodyScanOwnershipBackfillValidationError", "validation_error", 2),
        ("BodyScanOwnershipBackfillIdentityError", "identity_error", 1),
        ("BodyScanOwnershipBackfillDependencyError", "dependency_error", 1),
        ("BodyScanOwnershipBackfillStateError", "state_error", 1),
        ("BodyScanOwnershipBackfillProvenanceError", "provenance_error", 1),
        ("BodyScanOwnershipBackfillDuplicateError", "duplicate_error", 1),
    ],
)
...
def test_restore_blocked_apply_emits_only_typed_error_code(monkeypatch, capsys):
    ...
    async def fail(_session, *, batch_size):
        assert batch_size == 1
        raise cli.backfill_service.BodyScanOwnershipBackfillStateError(
