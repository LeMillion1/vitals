"""Non-PHI operator contract for the fixed Stage-3I DayContext CLI."""
...
from scripts import backfill_day_context_subject_ownership as cli
...
PHI_SENTINEL = "private-day-context-answer-sentinel"
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.DAY_CONTEXT_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
            ...
            # The service may grow its private projection; the CLI must never copy it.
            "subject_id": PHI_SENTINEL,
            "last_scanned_id": 12345,
            "scan_high_watermark_id": 98765,
            "date": PHI_SENTINEL,
            "answers": {"private": PHI_SENTINEL},
            "planned": {"private": PHI_SENTINEL},
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    process = subprocess.run(
        [
            sys.executable,
            str(REPOSITORY_ROOT / "scripts" / "backfill_day_context_subject_ownership.py"),
            "--help",
        ],
        cwd=REPOSITORY_ROOT,
        capture_output=True,
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.DAY_CONTEXT_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_allowlisted_aggregates(
    ...
    assert payload["operation"] == "day_context_subject_ownership_backfill"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.channel_optional.day_context.v1"],
        ["--table", "day_context"],
...
@pytest.mark.parametrize(
    ...
    [
        ("DayContextOwnershipBackfillValidationError", "validation_error", 2),
        ("DayContextOwnershipBackfillIdentityError", "identity_error", 1),
        ("DayContextOwnershipBackfillDependencyError", "dependency_error", 1),
        ("DayContextOwnershipBackfillStateError", "state_error", 1),
        ("DayContextOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
