"""Non-PHI operator contract for the fixed Stage-3C HRT-child CLI."""
...
from scripts import backfill_hrt_child_subject_ownership as cli
...
CATALOG = ("hrt_cycle_items", "hrt_cycle_template_items")
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_script_invocation_is_fixed_target_and_one_json_line():
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_safe_fields(monkeypatch, capsys):
    ...
    assert payload["operation"] == "hrt_child_subject_ownership_backfill"
    assert payload["phase"] == cli.backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_PHASE
...
def test_apply_uses_fresh_committed_sessions_and_stops_at_completion(
    ...
    results = iter(
        (
            ...
            _Result(
                ...
                unchanged_rows=1,
                remaining_rows=0,
                batch_table="hrt_cycle_template_items",
                batch_scanned_rows=2,
    ...
    assert payload["batch_table"] == "hrt_cycle_template_items"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.inherited_children.hrt.v1"],
        ["--table", "hrt_cycle_items"],
...
@pytest.mark.parametrize(
    ...
    [
        ("HrtChildOwnershipBackfillValidationError", "validation_error", 2),
        ("HrtChildOwnershipBackfillIdentityError", "identity_error", 1),
        ("HrtChildOwnershipBackfillDependencyError", "dependency_error", 1),
        ("HrtChildOwnershipBackfillStateError", "state_error", 1),
        ("HrtChildOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
