"""Non-PHI contract for the fixed Stage-3D provider/raw operator CLI."""
...
from scripts import backfill_provider_raw_subject_ownership as cli
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.PROVIDER_RAW_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
            ...
            "raw_payload_id": 12345,
            "integration_connection_id": PHI_SENTINEL,
            "external_id": PHI_SENTINEL,
            "date": PHI_SENTINEL,
...
def test_direct_script_invocation_is_fixed_target_and_one_json_line():
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.PROVIDER_RAW_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }


def test_default_is_read_only_and_projects_only_safe_fields(monkeypatch, capsys):
    ...
    async def apply(_session, *, batch_size):
        del batch_size
        calls["apply"] += 1
        raise AssertionError("status must not enter mutator")
    ...
    assert payload["operation"] == "provider_raw_subject_ownership_backfill"
    assert payload["mode"] == "status"
    assert payload["tables_total"] == 4
...
def test_apply_uses_fresh_committed_sessions_and_stops_at_completion(
    ...
    assert payload["batch_table"] == "hevy_workouts"
    assert payload["batches_processed"] == 2


def test_restore_blocked_status_is_safe_and_apply_failure_is_typed(
    monkeypatch,
    capsys,
):
    ...
    async def apply(_session, *, batch_size):
        del batch_size
        raise cli.backfill_service.ProviderRawOwnershipBackfillStateError(
            PHI_SENTINEL
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.provider_raw_linked.v1"],
        ["--table", "garmin_daily"],
...
@pytest.mark.parametrize(
    ...
    [
        ("ProviderRawOwnershipBackfillValidationError", "validation_error", 2),
        ("ProviderRawOwnershipBackfillIdentityError", "identity_error", 1),
        ("ProviderRawOwnershipBackfillDependencyError", "dependency_error", 1),
        ("ProviderRawOwnershipBackfillStateError", "state_error", 1),
        ("ProviderRawOwnershipBackfillProvenanceError", "provenance_error", 1),
        ("ProviderRawOwnershipBackfillDuplicateError", "duplicate_error", 1),
    ],
)
