"""Non-PHI operator contract for the fixed Stage-3F HRT-compound CLI."""
...
from scripts import backfill_hrt_compound_subject_ownership as cli
...
CATALOG = ("hrt_compounds", "hrt_compound_components")
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.HRT_COMPOUND_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_script_invocation_is_fixed_target_and_one_json_line():
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.HRT_COMPOUND_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_safe_fields(monkeypatch, capsys):
    ...
    assert payload["operation"] == "hrt_compound_subject_ownership_backfill"
...
def test_not_started_status_is_visible_without_becoming_completed(
    monkeypatch,
    capsys,
):
...
def test_apply_uses_fresh_committed_sessions_and_stops_at_completion(
    ...
    results = iter(
        (
            ...
            _Result(
                ...
                unchanged_rows=1,
                remaining_rows=0,
                batch_table="hrt_compound_components",
                batch_scanned_rows=2,
    ...
    assert payload["batch_table"] == "hrt_compound_components"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.mixed_catalog.hrt.v1"],
        ["--table", "hrt_compounds"],
...
@pytest.mark.parametrize(
    ...
    [
        ("HrtCompoundOwnershipBackfillValidationError", "validation_error", 2),
        ("HrtCompoundOwnershipBackfillIdentityError", "identity_error", 1),
        ("HrtCompoundOwnershipBackfillDependencyError", "dependency_error", 1),
        ("HrtCompoundOwnershipBackfillStateError", "state_error", 1),
        ("HrtCompoundOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
