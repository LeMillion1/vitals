"""Non-PHI operator contract for the fixed Stage-3L WeightLog CLI."""
...
from scripts import backfill_weight_log_subject_ownership as cli
...
PHI_SENTINEL = "private-weight-mass-date-note-raw-sentinel"
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.WEIGHT_LOG_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    process = subprocess.run(
        [
            sys.executable,
            str(REPOSITORY_ROOT / "scripts" / "backfill_weight_log_subject_ownership.py"),
            "--help",
        ],
        cwd=REPOSITORY_ROOT,
        capture_output=True,
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.WEIGHT_LOG_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_allowlisted_aggregates(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weight_log_ownership_backfill", preflight
    )
    monkeypatch.setattr(
        cli.backfill_service, "run_weight_log_ownership_backfill_batch", apply
    )
    ...
    assert payload["operation"] == "weight_log_subject_ownership_backfill"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.channel_optional.weight_logs.v1"],
        ["--table", "weight_logs"],
...
@pytest.mark.parametrize(
    ...
    [
        ("WeightLogOwnershipBackfillValidationError", "validation_error", 2),
        ("WeightLogOwnershipBackfillIdentityError", "identity_error", 1),
        ("WeightLogOwnershipBackfillDependencyError", "dependency_error", 1),
        ("WeightLogOwnershipBackfillStateError", "state_error", 1),
        ("WeightLogOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
def test_typed_failures_emit_bounded_codes(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weight_log_ownership_backfill", fail
    )
...
def test_unsafe_service_projection_is_sanitized(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weight_log_ownership_backfill", unsafe
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
...
def test_cancel_and_commit_failure_never_expose_details(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weight_log_ownership_backfill", cancelled
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
