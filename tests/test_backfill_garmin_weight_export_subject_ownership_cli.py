"""Non-PHI operator contract for the fixed Stage-3Q GarminWeightExport CLI."""
...
from scripts import backfill_garmin_weight_export_subject_ownership as cli
...
PHI_SENTINEL = "private-outbox-date-mass-sample-sentinel"
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
            ...
            "actor_user_id": UUID_SENTINEL,
            "integration_connection_id": UUID_SENTINEL,
            "raw_id": 98765,
            "Garmin weight export_id": 12345,
            "date": "2026-08-21",
            "key": PHI_SENTINEL,
            "note": PHI_SENTINEL,
            "batch_id": PHI_SENTINEL,
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    process = subprocess.run(
        [
            sys.executable,
            str(REPOSITORY_ROOT / "scripts" / "backfill_garmin_weight_export_subject_ownership.py"),
            "--help",
        ],
        cwd=REPOSITORY_ROOT,
        capture_output=True,
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_allowlisted_aggregates(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_garmin_weight_export_ownership_backfill", preflight
    )
    monkeypatch.setattr(
        cli.backfill_service, "run_garmin_weight_export_ownership_backfill_batch", apply
    )
    ...
    assert payload["operation"] == "garmin_weight_export_subject_ownership_backfill"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.provider_outbox.garmin_weight_exports.v1"],
...
@pytest.mark.parametrize(
    ...
    [
        ("GarminWeightExportOwnershipBackfillValidationError", "validation_error", 2),
        ("GarminWeightExportOwnershipBackfillIdentityError", "identity_error", 1),
        ("GarminWeightExportOwnershipBackfillDependencyError", "dependency_error", 1),
        ("GarminWeightExportOwnershipBackfillStateError", "state_error", 1),
        ("GarminWeightExportOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
def test_typed_failures_emit_bounded_codes(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_garmin_weight_export_ownership_backfill", fail
    )
...
def test_unsafe_service_projection_is_sanitized(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_garmin_weight_export_ownership_backfill", unsafe
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
...
def test_cancel_and_commit_failure_never_expose_details(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_garmin_weight_export_ownership_backfill", cancelled
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
