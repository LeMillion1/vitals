"""Non-PHI operator contract for the fixed Stage-3G conflict-rule CLI."""
...
from scripts import backfill_conflict_rule_subject_ownership as cli
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.CONFLICT_RULE_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
            ...
            "code": PHI_SENTINEL,
            "message": PHI_SENTINEL,
            "condition_a": {"private": PHI_SENTINEL},
            "condition_b": {"private": PHI_SENTINEL},
            "citations": [PHI_SENTINEL],
            "timestamp": PHI_SENTINEL,
            "created_at": PHI_SENTINEL,
            "updated_at": PHI_SENTINEL,
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    process = subprocess.run(
        [
            sys.executable,
            str(REPOSITORY_ROOT / "scripts" / "backfill_conflict_rule_subject_ownership.py"),
            "--help",
        ],
        cwd=REPOSITORY_ROOT,
        capture_output=True,
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.CONFLICT_RULE_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }


def test_default_is_read_only_and_projects_only_allowlisted_counts(monkeypatch, capsys):
    ...
    assert payload["operation"] == "conflict_rule_subject_ownership_backfill"
...
def test_not_started_status_is_visible_without_becoming_completed(monkeypatch, capsys):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.mixed_catalog.conflict_rules.v1"],
...
@pytest.mark.parametrize(
    ...
    [
        ("ConflictRuleOwnershipBackfillValidationError", "validation_error", 2),
        ("ConflictRuleOwnershipBackfillIdentityError", "identity_error", 1),
        ("ConflictRuleOwnershipBackfillDependencyError", "dependency_error", 1),
        ("ConflictRuleOwnershipBackfillStateError", "state_error", 1),
        ("ConflictRuleOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
