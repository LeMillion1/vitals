"""Non-PHI operator contract for the fixed Stage-3R WeeklyDigest CLI."""
...
from scripts import backfill_weekly_digest_subject_ownership as cli
...
PHI_SENTINEL = "private-digest-narrative-context-sentinel"
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.WEEKLY_DIGEST_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_invocation_is_fixed_target_and_one_safe_json_line():
    process = subprocess.run(
        [
            sys.executable,
            str(REPOSITORY_ROOT / "scripts" / "backfill_weekly_digest_subject_ownership.py"),
            "--help",
        ],
        cwd=REPOSITORY_ROOT,
        capture_output=True,
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.WEEKLY_DIGEST_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_allowlisted_aggregates(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weekly_digest_ownership_backfill", preflight
    )
    monkeypatch.setattr(
        cli.backfill_service, "run_weekly_digest_ownership_backfill_batch", apply
    )
    ...
    assert payload["operation"] == "weekly_digest_subject_ownership_backfill"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.retained_artifact.weekly_digests.v1"],
...
@pytest.mark.parametrize(
    ...
    [
        ("WeeklyDigestOwnershipBackfillValidationError", "validation_error", 2),
        ("WeeklyDigestOwnershipBackfillIdentityError", "identity_error", 1),
        ("WeeklyDigestOwnershipBackfillDependencyError", "dependency_error", 1),
        ("WeeklyDigestOwnershipBackfillStateError", "state_error", 1),
        ("WeeklyDigestOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
def test_typed_failures_emit_bounded_codes(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weekly_digest_ownership_backfill", fail
    )
...
def test_unsafe_service_projection_is_sanitized(
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weekly_digest_ownership_backfill", unsafe
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
...
def test_cancel_and_commit_failure_never_expose_details(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        cli.backfill_service, "preflight_weekly_digest_ownership_backfill", cancelled
    )
    assert cli.main([]) == 1
    payload, rendered = _output(capsys)
