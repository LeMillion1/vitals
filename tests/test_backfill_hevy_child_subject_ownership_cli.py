"""Non-PHI operator contract for the fixed Stage-3E Hevy-child CLI."""
...
from scripts import backfill_hevy_child_subject_ownership as cli
...
CATALOG = ("hevy_exercises", "hevy_sets")
...
class _Result:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "phase_key": cli.backfill_service.HEVY_CHILD_OWNERSHIP_BACKFILL_PHASE,
            "status": self.status,
...
def test_direct_script_invocation_is_fixed_target_and_one_json_line():
    ...
    assert json.loads(process.stdout) == {
        ...
        "phase": cli.backfill_service.HEVY_CHILD_OWNERSHIP_BACKFILL_PHASE,
        "result": "error",
    }
...
def test_default_is_read_only_and_projects_only_safe_fields(monkeypatch, capsys):
    ...
    assert payload["operation"] == "hevy_child_subject_ownership_backfill"
...
def test_restore_blocked_status_is_visible_without_becoming_completed(
    monkeypatch,
    capsys,
):
...
def test_apply_uses_fresh_committed_sessions_and_stops_at_completion(
    ...
    assert payload["batch_table"] == "hevy_sets"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--max-batches", "0"],
        ["--max-batches", "101"],
        ["--phase", "stage3.inherited_children.hevy.v1"],
        ["--table", "hevy_exercises"],
...
@pytest.mark.parametrize(
    ...
    [
        ("HevyChildOwnershipBackfillValidationError", "validation_error", 2),
        ("HevyChildOwnershipBackfillIdentityError", "identity_error", 1),
        ("HevyChildOwnershipBackfillDependencyError", "dependency_error", 1),
        ("HevyChildOwnershipBackfillStateError", "state_error", 1),
        ("HevyChildOwnershipBackfillProvenanceError", "provenance_error", 1),
    ],
)
