"""Production-shaped Stage-3Q rehearsal from a synthetic revision-0034 lake."""
...
WEIGHT_IDS = (58_050, 58_051)
EXPORT_IDS = (58_201, 58_202)
EXPORT_DATES = (date(2026, 8, 19), date(2026, 8, 20))
LIVE_DATE = date(2026, 8, 21)
LIVE_EXPORT_ID = 58_301
PRIVATE_SENTINEL = "synthetic-private-stage3q-outbox-error"
DOWNGRADE_REFUSAL = (
...
def _seed_revision_0034(connection: sa.Connection) -> None:
    tables = _reflect(connection, ("weight_logs", "garmin_weight_exports"))
    ...
    connection.execute(
        ...
        [
            ...
            for weight_id, on_date in zip(WEIGHT_IDS, EXPORT_DATES, strict=True)
        ],
    )
    connection.execute(
        tables["garmin_weight_exports"].insert(),
        [
            {
                "id": export_id,
                "date": on_date,
                "weight_log_id": weight_id,
                "weight_kg": 81.5,
                "measured_at": datetime(on_date.year, on_date.month, on_date.day, 7),
                "status": status,
                "attempts": 0,
                "remote_owned": False,
                "last_error": PRIVATE_SENTINEL,
                "created_at": created_at,
                "updated_at": updated_at,
            }
            for export_id, weight_id, on_date, status in zip(
                EXPORT_IDS,
                WEIGHT_IDS,
                EXPORT_DATES,
                ("pending", "sent"),
    ...
    for table_name, value in (
        ...
        ("garmin_weight_exports", max(EXPORT_IDS)),
    ):
        connection.execute(
            sa.text(
...
def _outbox_nonownership_hash(connection: sa.Connection) -> str:
    table = _reflect(connection, ("garmin_weight_exports",))["garmin_weight_exports"]
    rows = [
        dict(row)
        for row in connection.execute(
            sa.select(
                table.c.id,
                table.c.date,
                table.c.weight_log_id,
                table.c.weight_kg,
                table.c.measured_at,
                table.c.status,
                table.c.attempts,
                table.c.remote_owned,
                table.c.last_error,
                table.c.created_at,
                table.c.updated_at,
            )
            .where(table.c.id.in_(EXPORT_IDS))
            .order_by(table.c.id)
        ).mappings()
    ]
    return _digest(rows)
...
async def _bootstrap_roots(engine: AsyncEngine):
    ...
    async with factory() as session:
        identity = await bootstrap_legacy_owner(
            session,
            username="synthetic-stage3q-owner",
...
async def _run_cli(
    ...
    *,
    ...
    assert not any(day.isoformat() in rendered for day in EXPORT_DATES)
    assert not any(str(value) in rendered for value in EXPORT_IDS + WEIGHT_IDS)
    return payload
...
async def _checkpoint_states(engine: AsyncEngine) -> list[dict[str, Any]]:
    async with engine.connect() as connection:
        rows = await connection.execute(
            ...
            {
                "phase": GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE,
                "prefix": f"{GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE}.%",
            },
        )
        return [dict(row) for row in rows.mappings()]


async def _outbox_graph(engine: AsyncEngine) -> list[dict[str, Any]]:
    async with engine.connect() as connection:
        rows = await connection.execute(
            sa.text(
                "SELECT id, subject_id, integration_connection_id, "
                "requested_by_user_id, weight_log_id, date, weight_kg, status, "
                "attempts, remote_owned, created_at, updated_at "
                "FROM garmin_weight_exports ORDER BY id"
            )
        )
        return [dict(row) for row in rows.mappings()]
...
@pytest.mark.asyncio
async def test_real_postgres_0034_garmin_weight_export_stop_resume_volatility_and_restore(
    db_session,
    monkeypatch,
):
    ...
    try:
        ...
        business_hash_before = await _run_sync(engine, _outbox_nonownership_hash)
        identity = await _bootstrap_roots(engine)
        await _sync_catalogs(engine)
        async with engine.connect() as connection:
            destination_id = await connection.scalar(
                sa.text(
                    "SELECT id FROM integration_connections "
                    "WHERE subject_id=:subject AND provider='garmin' "
                    "AND connection_type='account'"
                ),
        ...
        assert status["phase"] == GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE
        ...
        assert started["batch_table"] == "garmin_weight_exports"
        assert started["batch_scanned_rows"] == 1
        assert started["batch_updated_rows"] == 1
        assert await _run_sync(engine, _outbox_nonownership_hash) == business_hash_before
        ...
        graph = await _outbox_graph(engine)
        ...
        graph = await _outbox_graph(engine)
        assert {row["id"] for row in graph} == set(EXPORT_IDS)
        assert all(row["subject_id"] == identity.subject_id for row in graph)

        assert all(
            row["integration_connection_id"] == destination_id
            for row in await _outbox_graph(engine)
        )
        ...
        # Supported post-completion volatility: a strict live outbox row with
        # the complete destination graph, above the frozen high-water mark.
        async with engine.begin() as connection:
            await connection.execute(
                sa.text(
                    "INSERT INTO garmin_weight_exports "
                    "(id, subject_id, integration_connection_id, date, weight_kg, "
                    "measured_at, status, attempts, remote_owned) "
                    "VALUES (:id, :subject, :connection, :date, 80.4, "
                    ":measured_at, 'delete_pending', 0, false)"
                ),
                {
                    "id": LIVE_EXPORT_ID,
                    "subject": identity.subject_id,
                    ...
                    "date": LIVE_DATE,
                    "measured_at": datetime(2026, 8, 21, 7),
                },
            )
        ...
        restored_graph = await _outbox_graph(engine)
        assert len(restored_graph) == 3
        assert all(row["subject_id"] == identity.subject_id for row in restored_graph)
        # Backup v1 carries neither the destination account nor the requester.
        assert all(
            row["integration_connection_id"] is None for row in restored_graph
        )
        assert all(row["requested_by_user_id"] is None for row in restored_graph)
        ...
        assert await _phase_statuses(
            engine, GARMIN_WEIGHT_EXPORT_OWNERSHIP_BACKFILL_PHASE
        ) == ("restore_blocked",)
        ...
        assert await _outbox_graph(engine) == []
