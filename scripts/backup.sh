#!/bin/sh
# Periodic backup for Vitals: the PostgreSQL database + the Garmin token store.
#
...
# /backups, archives the Garmin session volume next to it, then prunes both
# older than the retention window. One pass on start, then once every 24h.
#
...
while true; do
    ...
    deleted="$(find "$BACKUP_DIR" \( -name 'vitals_*.sql.gz' -o -name 'garmin_session_*.tar.gz' \) -type f -mtime +"$RETENTION_DAYS" -print -delete | wc -l)"
