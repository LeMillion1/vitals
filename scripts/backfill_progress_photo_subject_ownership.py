#!/usr/bin/env python3
"""Inspect or advance the bounded progress-photo ownership backfill.

The default invocation is read-only. ``--apply`` is required before one or
more independently committed batches may mutate data. This fixed Stage-3H
...
from vitals.services import progress_photo_ownership_backfill_service as backfill_service
...
def build_parser() -> argparse.ArgumentParser:
    """Return the fixed-target progress-photo argument parser."""
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the fixed progress-photo ownership checkpoint",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=(
                backfill_service.MAX_PROGRESS_PHOTO_OWNERSHIP_BACKFILL_BATCH_SIZE
            ),
        ),
        default=(
            backfill_service.DEFAULT_PROGRESS_PHOTO_OWNERSHIP_BACKFILL_BATCH_SIZE
        ),
        metavar=(
            "1.."
            f"{backfill_service.MAX_PROGRESS_PHOTO_OWNERSHIP_BACKFILL_BATCH_SIZE}"
        ),
    )
    parser.add_argument(
        "--max-batches",
...
def _catalog_tables() -> tuple[str, ...]:
    catalog = backfill_service.PROGRESS_PHOTO_OWNERSHIP_BACKFILL_TABLES
    if not isinstance(catalog, tuple) or catalog != ("progress_photos",):
        raise RuntimeError("backfill service returned an unsafe projection")
    return catalog
...
def _success_payload(
    ...
    *,
    ...
    """Apply the CLI's strict no-ID/no-PHI allowlist."""

    safe = result.to_safe_dict()
    if not isinstance(safe, dict):
        raise RuntimeError("backfill service returned an unsafe projection")
    phase_key = safe.get("phase_key")
    if phase_key != backfill_service.PROGRESS_PHOTO_OWNERSHIP_BACKFILL_PHASE:
        raise RuntimeError("backfill service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in backfill_service.ProgressPhotoOwnershipBackfillStatus
    }
    if allowed_statuses != {
        "not_started",
        "running",
    ...
    if status == "completed" and (
        ...
        raise RuntimeError("backfill service returned an unsafe projection")
    if status == "restore_blocked" and (
        payload["completed_tables"] != 0
        or payload["scanned_rows"] != 0
        or payload["updated_rows"] != 0
        or payload["unchanged_rows"] != 0
        or payload["remaining_rows"] != payload["snapshot_rows"]
    ):
        raise RuntimeError("backfill service returned an unsafe projection")
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = (
                    await backfill_service.preflight_progress_photo_ownership_backfill(
                        session
                    )
                )
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = (
                    await backfill_service.run_progress_photo_ownership_backfill_batch(
                        session,
                        batch_size=args.batch_size,
                    )
                )
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("ProgressPhotoOwnershipBackfillValidationError", "validation_error"),
        ("ProgressPhotoOwnershipBackfillIdentityError", "identity_error"),
        ("ProgressPhotoOwnershipBackfillDependencyError", "dependency_error"),
        ("ProgressPhotoOwnershipBackfillStateError", "state_error"),
        ("ProgressPhotoOwnershipBackfillProvenanceError", "provenance_error"),
        ("ProgressPhotoOwnershipBackfillDuplicateError", "duplicate_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "ProgressPhotoOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.PROGRESS_PHOTO_OWNERSHIP_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
