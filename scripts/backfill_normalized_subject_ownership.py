#!/usr/bin/env python3
"""Inspect or advance the bounded normalized-manual ownership backfill.

The default invocation is read-only. ``--apply`` is required before one or
more independently committed batches may mutate data. This Stage-3B command
has one reviewed phase and table catalog; it deliberately has no table, phase,
reset, delete, or database-URL selector.
"""
...
def build_parser() -> argparse.ArgumentParser:
    """Return the fixed-target Stage-3B argument parser."""
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the normalized-manual ownership checkpoint group",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=(
                backfill_service.MAX_NORMALIZED_OWNERSHIP_BACKFILL_BATCH_SIZE
            ),
        ),
        default=(
            backfill_service.DEFAULT_NORMALIZED_OWNERSHIP_BACKFILL_BATCH_SIZE
        ),
        metavar=(
            "1.."
            f"{backfill_service.MAX_NORMALIZED_OWNERSHIP_BACKFILL_BATCH_SIZE}"
        ),
    )
    parser.add_argument(
        "--max-batches",
...
def _catalog_tables() -> tuple[str, ...]:
    catalog = getattr(backfill_service, "NORMALIZED_MANUAL_TABLES", None)
    if catalog is None:
        catalog = getattr(
            backfill_service,
    ...
    if not isinstance(catalog, tuple) or not catalog:
        raise RuntimeError("backfill service returned an unsafe projection")
    if any(not isinstance(table, str) or not table for table in catalog):
        raise RuntimeError("backfill service returned an unsafe projection")
...
def _success_payload(
    ...
    *,
    ...
    if phase_key != backfill_service.NORMALIZED_MANUAL_BACKFILL_PHASE:
        raise RuntimeError("backfill service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in backfill_service.NormalizedOwnershipBackfillStatus
    }
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = (
                    await backfill_service.preflight_normalized_ownership_backfill(
                        session
                    )
                )
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = (
                    await backfill_service.run_normalized_ownership_backfill_batch(
                        session,
                        batch_size=args.batch_size,
                    )
                )
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("NormalizedOwnershipBackfillValidationError", "validation_error"),
        ("NormalizedOwnershipBackfillIdentityError", "identity_error"),
        ("NormalizedOwnershipBackfillDependencyError", "dependency_error"),
        ("NormalizedOwnershipBackfillStateError", "state_error"),
        ("NormalizedOwnershipBackfillProvenanceError", "provenance_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "NormalizedOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.NORMALIZED_MANUAL_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
