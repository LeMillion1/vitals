#!/usr/bin/env python3
"""Inspect or advance the fixed Stage-3P BodyScanMetric ownership backfill.
...
from vitals.services import body_scan_metric_ownership_backfill_service as backfill_service


OUTPUT_FORMAT_VERSION = 1
OPERATION = "body_scan_metric_subject_ownership_backfill"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the fixed body-scan metric ownership checkpoint",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=backfill_service.MAX_BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_BATCH_SIZE,
        ),
        default=backfill_service.DEFAULT_BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_BATCH_SIZE,
        metavar=f"1..{backfill_service.MAX_BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_BATCH_SIZE}",
    )
    parser.add_argument(
        "--max-batches",
...
def _catalog_tables() -> tuple[str, ...]:
    catalog = backfill_service.BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_TABLES
    if not isinstance(catalog, tuple) or catalog != ("body_scan_metrics",):
        raise RuntimeError("backfill service returned an unsafe projection")
    return catalog
...
def _success_payload(
    ...
    *,
    ...
    if phase_key != backfill_service.BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_PHASE:
        raise RuntimeError("backfill service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in backfill_service.BodyScanMetricOwnershipBackfillStatus
    }
    if allowed_statuses != {"not_started", "running", "completed"}:
        raise RuntimeError("backfill service returned an unsafe projection")
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = await backfill_service.preflight_body_scan_metric_ownership_backfill(
                    session
                )
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = await backfill_service.run_body_scan_metric_ownership_backfill_batch(
                    session,
                    batch_size=args.batch_size,
                )
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("BodyScanMetricOwnershipBackfillValidationError", "validation_error"),
        ("BodyScanMetricOwnershipBackfillIdentityError", "identity_error"),
        ("BodyScanMetricOwnershipBackfillDependencyError", "dependency_error"),
        ("BodyScanMetricOwnershipBackfillStateError", "state_error"),
        ("BodyScanMetricOwnershipBackfillProvenanceError", "provenance_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "BodyScanMetricOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.BODY_SCAN_METRIC_OWNERSHIP_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
