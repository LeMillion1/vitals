#!/usr/bin/env python3
"""Inspect or record the fixed Stage-5A scoped-key cutover audit.

The audit proves, read-only, that every reviewed scoped key can replace its
legacy global key: that no row would collide under the proposed key and that no
row is missing the scope the key depends on. ``--apply`` records the reviewed
evidence. No table, key, phase, reset, delete, or database-URL selector is
exposed, and the operation never creates, drops, or writes anything but its own
checkpoint.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="record the reviewed scoped-key cutover audit evidence",
    )
    return parser
...
def _nonnegative_count(safe: dict[str, Any], key: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise RuntimeError("audit service returned an unsafe projection")
    return value
...
def _success_payload(result: Any, *, mode: str) -> dict[str, Any]:
    ...
    if not isinstance(safe, dict):
        raise RuntimeError("audit service returned an unsafe projection")
    if safe.get("phase_key") != audit_service.SCOPED_KEY_AUDIT_PHASE:
        raise RuntimeError("audit service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in audit_service.ScopedKeyAuditStatus
    }
    ...
    if not isinstance(status, str) or status not in allowed_statuses:
        raise RuntimeError("audit service returned an unsafe projection")
    checksum = safe.get("audit_checksum")
    ...
    if payload["collisions_total"] != 0 or payload["unscoped_rows_total"] != 0:
        raise RuntimeError("audit service returned an unsafe projection")
    if payload["scoped_indexes_total"] < payload["legacy_keys_total"]:
        raise RuntimeError("audit service returned an unsafe projection")
    if payload["legacy_keys_total"] == 0:
        raise RuntimeError("audit service returned an unsafe projection")
    return payload
...
async def _execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        async with factory() as session:
            if not args.apply:
                result = await audit_service.preflight_scoped_key_audit(session)
                payload = _success_payload(result, mode="status")
                await session.rollback()
                return payload
            result = await audit_service.run_scoped_key_audit(session)
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("ScopedKeyAuditCollision", "collision"),
        ("ScopedKeyAuditDependencyError", "dependency_error"),
        ("ScopedKeyAuditStateError", "state_error"),
    )
    for class_name, code in typed_errors:
        error_type = getattr(audit_service, class_name, None)
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(audit_service, "ScopedKeyAuditError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": audit_service.SCOPED_KEY_AUDIT_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        exit_code = 2 if typed_code == "collision" else 1
