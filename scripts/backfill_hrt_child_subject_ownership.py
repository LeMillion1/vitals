#!/usr/bin/env python3
"""Inspect or advance the bounded HRT-child subject-ownership backfill.

The default invocation is read-only. ``--apply`` is required before one or
more independently committed batches may mutate data. This fixed Stage-3C
...
from vitals.services import hrt_child_ownership_backfill_service as backfill_service


OUTPUT_FORMAT_VERSION = 1
OPERATION = "hrt_child_subject_ownership_backfill"
...
def build_parser() -> argparse.ArgumentParser:
    """Return the fixed-target HRT-child argument parser."""
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the fixed HRT-child ownership checkpoint group",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=(
                backfill_service.MAX_HRT_CHILD_OWNERSHIP_BACKFILL_BATCH_SIZE
            ),
        ),
        default=(
            backfill_service.DEFAULT_HRT_CHILD_OWNERSHIP_BACKFILL_BATCH_SIZE
        ),
        metavar=(
            "1.."
            f"{backfill_service.MAX_HRT_CHILD_OWNERSHIP_BACKFILL_BATCH_SIZE}"
        ),
    )
    parser.add_argument(
        "--max-batches",
...
def _catalog_tables() -> tuple[str, ...]:
    catalog = backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_TABLES
...
def _success_payload(
    ...
    *,
    ...
    if phase_key != backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_PHASE:
        raise RuntimeError("backfill service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in backfill_service.HrtChildOwnershipBackfillStatus
    }
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = await backfill_service.preflight_hrt_child_ownership_backfill(
                    session
                )
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = (
                    await backfill_service.run_hrt_child_ownership_backfill_batch(
                        session,
                        batch_size=args.batch_size,
                    )
                )
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("HrtChildOwnershipBackfillValidationError", "validation_error"),
        ("HrtChildOwnershipBackfillIdentityError", "identity_error"),
        ("HrtChildOwnershipBackfillDependencyError", "dependency_error"),
        ("HrtChildOwnershipBackfillStateError", "state_error"),
        ("HrtChildOwnershipBackfillProvenanceError", "provenance_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "HrtChildOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.HRT_CHILD_OWNERSHIP_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
