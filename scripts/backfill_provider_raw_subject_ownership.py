#!/usr/bin/env python3
"""Inspect or advance the fixed provider/raw-linked ownership backfill.

The default invocation is read-only. ``--apply`` is required before one or
more independently committed batches may mutate data. The command exposes no
table, phase, reset, delete, or database-URL selector.
"""
...
from vitals.services import provider_raw_ownership_backfill_service as backfill_service


OUTPUT_FORMAT_VERSION = 1
OPERATION = "provider_raw_subject_ownership_backfill"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the fixed provider/raw-linked ownership checkpoint group",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=(
                backfill_service.MAX_PROVIDER_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE
            ),
        ),
        default=(
            backfill_service.DEFAULT_PROVIDER_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE
        ),
        metavar=(
            "1.."
            f"{backfill_service.MAX_PROVIDER_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE}"
        ),
    )
    parser.add_argument(
        "--max-batches",
...
def _catalog_tables() -> tuple[str, ...]:
    catalog = backfill_service.PROVIDER_RAW_OWNERSHIP_BACKFILL_TABLES
    if not isinstance(catalog, tuple) or catalog != _FIXED_TABLES:
...
def _success_payload(
    ...
    *,
    ...
    if phase_key != backfill_service.PROVIDER_RAW_OWNERSHIP_BACKFILL_PHASE:
        raise RuntimeError("backfill service returned an unsafe projection")
    allowed_statuses = {
        item.value for item in backfill_service.ProviderRawOwnershipBackfillStatus
    }
    if allowed_statuses != {
        "not_started",
        "running",
    ...
    if batch_table is not None and (
        not isinstance(batch_table, str) or batch_table not in _catalog_tables()
    ):
        raise RuntimeError("backfill service returned an unsafe projection")
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = (
                    await backfill_service.preflight_provider_raw_ownership_backfill(
                        session
                    )
                )
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = (
                    await backfill_service.run_provider_raw_ownership_backfill_batch(
                        session,
                        batch_size=args.batch_size,
                    )
                )
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("ProviderRawOwnershipBackfillValidationError", "validation_error"),
        ("ProviderRawOwnershipBackfillIdentityError", "identity_error"),
        ("ProviderRawOwnershipBackfillDependencyError", "dependency_error"),
        ("ProviderRawOwnershipBackfillStateError", "state_error"),
        ("ProviderRawOwnershipBackfillProvenanceError", "provenance_error"),
        ("ProviderRawOwnershipBackfillDuplicateError", "duplicate_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "ProviderRawOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.PROVIDER_RAW_OWNERSHIP_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
