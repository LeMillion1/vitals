#!/usr/bin/env python3
"""Inspect or advance the bounded legacy raw-payload ownership backfill.

The default invocation is read-only.  ``--apply`` is required before one or
more independently committed batches may mutate data.  This Stage-3A command
has one reviewed phase only; it deliberately has no table, phase, reset,
delete, or database-URL selector.
...
# the import path.  Resolve the repository root from this reviewed file, never
...
class _SafeArgumentParser(argparse.ArgumentParser):
    def error(self, _message: str) -> None:
        # argparse normally echoes the invalid value and full argv to stderr.
        # An operator can accidentally paste a DB URL or another secret, so the
        # machine boundary emits only a fixed code from ``main`` instead.
...
def build_parser() -> argparse.ArgumentParser:
    """Return the fixed-target Stage-3A argument parser."""

    # This is a machine-facing maintenance boundary: even help is represented
    # by the same one-line JSON error envelope instead of a second plaintext
    # output format.  Operational examples live in the reviewed docs.
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="advance the raw-payload ownership checkpoint",
    )
    parser.add_argument(
        "--batch-size",
        type=_bounded_positive_int(
            option="--batch-size",
            maximum=backfill_service.MAX_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE,
        ),
        default=backfill_service.DEFAULT_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE,
        metavar=f"1..{backfill_service.MAX_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE}",
        help=(
            "rows per transaction "
            f"(default {backfill_service.DEFAULT_RAW_OWNERSHIP_BACKFILL_BATCH_SIZE})"
        ),
    )
    parser.add_argument(
        "--max-batches",
        ...
        help="maximum independently committed batches (default 1)",
    )
    return parser
...
def _success_payload(
    ...
    *,
    ...
    """Apply the CLI's stricter no-ID allowlist to the service projection."""
    ...
    if (
        ...
        or phase_key != backfill_service.RAW_OWNERSHIP_BACKFILL_PHASE
    ):
        raise RuntimeError("backfill service returned an unsafe projection")
    status = safe.get("status")
    allowed_statuses = {
        item.value for item in backfill_service.RawOwnershipBackfillStatus
    }
    if allowed_statuses != {
        "not_started",
        "running",
    ...
    for key in _CUMULATIVE_COUNT_KEYS:
    ...
    try:
        if not args.apply:
            async with factory() as session:
                result = await backfill_service.preflight_raw_ownership_backfill(
                    session
                )
                # Even a read opens a transaction on PostgreSQL.  Explicitly
                # close it without creating checkpoint state.
        ...
        for _batch_number in range(args.max_batches):
            async with factory() as session:
                result = (
                    await backfill_service.run_raw_ownership_backfill_batch(
                        session,
                        batch_size=args.batch_size,
                    )
                )
                ...
                # Each batch owns a separate durable checkpoint transaction.
                await session.commit()
            batches_processed += 1
            payload = candidate_payload
            if payload["completed"]:
                break
        if payload is None:  # Defensive: argparse never accepts zero batches.
...
def _typed_error_code(exc: Exception) -> str | None:
    typed_errors = (
        ("RawOwnershipBackfillValidationError", "validation_error"),
        ("RawOwnershipBackfillIdentityError", "identity_error"),
        ("RawOwnershipBackfillStateError", "state_error"),
        ("RawOwnershipBackfillMappingError", "mapping_error"),
        ("RawOwnershipBackfillDuplicateError", "duplicate_error"),
    )
    for class_name, code in typed_errors:
        ...
        if isinstance(error_type, type) and isinstance(exc, error_type):
            return code
    base_type = getattr(backfill_service, "RawOwnershipBackfillError", None)
...
def _error_payload(*, mode: str, code: str) -> dict[str, Any]:
    return {
        "format_version": OUTPUT_FORMAT_VERSION,
        "operation": OPERATION,
        "phase": backfill_service.RAW_OWNERSHIP_BACKFILL_PHASE,
        "mode": mode,
        "result": "error",
        "error_code": code,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        payload = _error_payload(
            mode=mode,
            code=typed_code or "internal_error",
        )
